...
if ( !file_exists($dbFile) ) {
	...
	try {
		...
		$db->query("BEGIN;
			...
			COMMIT;");
			error_log("\n".'SQLite Database created '.$dbFile, 3, "cms.log");
	} catch (Exception $e) {
		die($error);
	}
} else {
...
if ($exists == true) {
	error_log("\n".'update data for page '.$pageId.' and contentId '.$contentId, 3, "cms.log");
	$query = "BEGIN;
		UPDATE aloha SET
			content = '".$content."'
		WHERE
	...
	error_log("\n".'insert data for page '.$pageId.' and contentId '.$contentId, 3, "cms.log");
	$query = "BEGIN;
		INSERT INTO aloha 
			(id, pageId, contentId, content)
		VALUES
